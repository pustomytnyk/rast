...
// @include     https://uk.wikipedia.org/w/index.php?title=*&action=edit
...
// в цій термінології спецсимволи (slots) розподілені підмножинах (subsets), які відображаються на панелях (panels)

// для діалогових вікон використовується https://www.mediawiki.org/wiki/OOjs_UI

// "unsafeWindow" це оригінальний контекст сторінки (звичаєвий контекст — від TamperMonkey). Можливо це можна видалити з допомогою "@grant none", не пробував http://wiki.greasespot.net/Content_Script_Injection

// manage dependencies like a boss
window.addPostponedFunc = function(requiredFlags, func) {
  window.funcRequirements = window.funcRequirements || [];
  window.funcRequirements.push({ flags: requiredFlags, func: func });
  firePostponedFuncs();
};

window.tryFirePostponedFunc = function(funcIndex) {
  if (!$.isArray(window.initializationFlags))
    return;
    var funcReq = window.funcRequirements[funcIndex];
  var i, len = funcReq.flags.length;
  for(i = 0; i < len; i++)
    if ($.inArray(funcReq.flags[i], window.initializationFlags) < 0)
      return;
    setTimeout(funcReq.func, 0); // prevent race condition
  return true;
};

window.firePostponedFuncs = function() {
  if(window.funcRequirements){
    window.funcRequirements = $.grep(window.funcRequirements, function(_, funcIndex){
      return tryFirePostponedFunc(funcIndex);
    }, true);
  }
};
...
$ ->
  mw.loader.using ['jquery.ui.droppable'], ->
...
$ ->
  ...
  importScript('User:AS/textarea.js')
  importScript('User:AS/TextareaHelper.js')
  ...
  addInitializationFlag 'EditTools:ready'
  addPostponedFunc ['textarea:ready', 'TextareaHelper:ready'], ->
    ...
    mw.loader.using ['mediawiki.cookie', 'oojs-ui'], ->
      EditTools.init()
      addInitializationFlag('EditTools:initialized')
  return
